...
class VoxelGeometryMaps:
    """Static axis / DN lookup tables and voxel↔world coordinate helpers."""
    ...
    # Nominal diameter (m) → outer diameter (m), per ASME B36.10M
    ...
    def outer_diameter_m(cls, nominal_diameter_m: float) -> float:
        best = min(cls.OD_BY_NOMINAL_M.keys(), key=lambda x: abs(x - nominal_diameter_m))
        return cls.OD_BY_NOMINAL_M[best]
