...
class SequentialMultiLineRouter(IMultiLineRouter):
    ...
    def route_all_lines(
        ...
        static_forbidden = self._custom_module_blocked_voxels(router_input, placed_nodes)
    ...
    def _custom_module_blocked_voxels(
        ...
        for nodes in groups.values():
            ...
            center_vc = self._wc_to_vc(center_wc)
    ...
    @staticmethod
    def _wc_to_vc(wc: tuple[float, float, float]) -> Vc:
        # Router config currently uses fixed origin=(0,0,0), voxel_size=0.2.
        vx = int(round(wc[0] / 0.2 - 0.5))
        vy = int(round(wc[1] / 0.2 - 0.5))
        vz = int(round(wc[2] / 0.2 - 0.5))
        return (vx, vy, vz)
